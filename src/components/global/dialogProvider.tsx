...
import {backDropAction} from "@/store/slice";

const DialogProvider = ()=>{
	// Use a selector to get the backdrop state from the Redux store
	...
	return <>
		...
		{/* Conditionally render a backdrop with a loading spinner if isOpenBackDrop is true */}
		{isOpenBackDrop&&<div
				onClick={()=>dispatch(backDropAction(false))}
				className="fixed items-center flex bg-gray-500 opacity-40 w-full h-full left-0 right-0 top-0 bottom-0"
				style={{zIndex:100000}}
		>
